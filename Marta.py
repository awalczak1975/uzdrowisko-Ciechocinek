...
st.markdown("""
    ...
    .chat-container { display: flex; flex-direction: column; gap: 10px; padding: 20px; background: #f8fafc; border-radius: 15px; border: 1px solid #e2e8f0; height: 500px; overflow-y: auto; }
    .bubble { padding: 12px 16px; border-radius: 18px; font-size: 14px; max-width: 75%; line-height: 1.4; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
    .bubble-me { align-self: flex-end; background-color: #eab308; color: #1e293b; font-weight: 600; border-bottom-right-radius: 2px; }
    .bubble-other { align-self: flex-start; background-color: white; color: #334155; border: 1px solid #e2e8f0; border-bottom-left-radius: 2px; }
    .chat-meta { font-size: 10px; margin-top: 5px; opacity: 0.7; }
    </style>
...
df_chat_raw = pobierz_arkusz("CZAT")

# Sprawdzenie nowych wiadomości
nowe_wiadomosci = False
if not df_chat_raw.empty and 'ODBIORCA' in df_chat_raw.columns:
    nowe_wiadomosci = not df_chat_raw[(df_chat_raw['ODBIORCA'] == zalogowany) & (df_chat_raw['STATUS'] == "NIEPRZECZYTANE")].empty
...
# 4. WIDOK GŁÓWNY
# ==========================================================
label_czat = f"💬 CZAT {'🔴' if nowe_wiadomosci else ''}" # Czerwona kropka powiadomienia
...
for i, nazwa in enumerate(["Zadania bieżące", "Zadania zrealizowane", "Terminy Sławka"]):
    with tabs[i]:
        ...
        if not df_tab.empty:
            ...
            m3.metric("✅ Zrealizowane", len(df_zreal_raw) if not df_zreal_raw.empty else 0)
...
# --- SEKCJA CZATU (DZIAŁAJĄCA) ---
with tabs[3]:
    st.subheader("💬 Messenger Firmowy")
    if not df_chat_raw.empty:
        # Filtracja wiadomości dla zalogowanego
        moj_czat = df_chat_raw[(df_chat_raw['NADAWCA'] == zalogowany) | (df_chat_raw['ODBIORCA'] == zalogowany)].tail(20)
        
        chat_html = '<div class="chat-container">'
        for _, msg in moj_czat.iterrows():
            jestem_nadawca = msg['NADAWCA'] == zalogowany
            klasa = "bubble-me" if jestem_nadawca else "bubble-other"
            chat_html += f'''
            <div class="bubble {klasa}">
                <div style="font-weight:800; font-size:11px;">{msg['NADAWCA']}</div>
                {msg['TREŚĆ']}
                <div class="chat-meta">{msg['DATA']}</div>
            </div>
            '''
        chat_html += '</div>'
    ...
    # Formularz wysyłania
    with st.form("wyslij_wiadomosc", clear_on_submit=True):
        c_target, c_msg, c_btn = st.columns([2, 5, 1])
        odbiorca = c_target.selectbox("Do:", [u for u in USERS.keys() if u != zalogowany])
        tresc = c_msg.text_input("Napisz wiadomość...")
        if c_btn.form_submit_button("WYŚLIJ"):
            if tresc:
                nowa_linia = [now_pl.strftime("%Y-%m-%d %H:%M"), zalogowany, odbiorca, tresc, "NIEPRZECZYTANE"]
                polacz().open("Marta-Dział Techniczny").worksheet("CZAT").append_row(nowa_linia)
