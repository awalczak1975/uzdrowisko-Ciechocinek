...
def pobierz_dane_po_indeksie(numer_arkusza):
    ...
    try:
        ...
        # Pobieramy arkusz po jego kolejności (0 = pierwszy, 1 = drugi itd.)
        if len(arkusze) > numer_arkusza:
            ...
            # Liczymy rzędy, które mają cokolwiek wpisane w pierwszej kolumnie
...
# --- 2. POBIERANIE DANYCH (PO KOLEJNOŚCI W ARKUSZU) ---
df_biezace, liczba_b, nazwa_b = pobierz_dane_po_indeksie(0)  # Pierwsza zakładka
df_zrealizowane, liczba_z, nazwa_z = pobierz_dane_po_indeksie(1) # Druga zakładka
df_slawka, _, nazwa_s = pobierz_dane_po_indeksie(4) # Zakładka nr 5 (Terminy Sławka)
...
with tabs[0]:
    if not df_biezace.empty: st.dataframe(df_biezace, use_container_width=True)
    else: st.info("Brak danych w pierwszej zakładce.")

with tabs[1]:
    if not df_zrealizowane.empty: st.dataframe(df_zrealizowane, use_container_width=True)
    else: st.info("Brak danych w drugiej zakładce.")

with tabs[2]:
    if not df_slawka.empty: st.dataframe(df_slawka, use_container_width=True)
    else: st.info("Brak danych w zakładce Sławka.")
