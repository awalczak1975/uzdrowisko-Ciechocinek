...
PLIK_KLUCZA = os.path.join(BASE_DIR, "klucz.json")
...
user_url = query_params.get("user", "Andrzej")
...
st.markdown("""
    ...
    .stTabs [data-baseweb="tab-list"] { border-bottom: 2px solid #eab308 !important; width: 100%; gap: 0px !important; }
    .stTabs [data-baseweb="tab"] { height: 36px; background-color: #f1f5f9; border-radius: 5px 5px 0 0; padding: 0px 20px; font-weight: bold; color: #475569; font-size: 0.85rem; border: none !important; margin-right: 0px !important; }
...
def pobierz_polaczenie():
    creds = ServiceAccountCredentials.from_json_keyfile_name(PLIK_KLUCZA, ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"])
...
def pobierz_dane(zakladka):
    try:
        ...
        # LICZNIK Z KOLUMNY A
        # Liczymy tylko wiersze, które mają jakikolwiek tekst w pierwszej kolumnie
...
with st.sidebar:
    ...
    if user_url == "Andrzej":
        ...
        if st.button("🔄 SYNCHRONIZUJ", use_container_width=True): st.rerun()
...
if not df_biezace.empty:
    ...
    with m1: 
        # LICZNIK BIEŻĄCYCH Z KOLUMNY A
        st.markdown(f'<div class="metric-card"><p>📋 MOJE ZADANIA</p><h3>{liczba_biezacych}</h3></div>', unsafe_allow_html=True)
    with m2:
    ...
    with m4:
        # LICZNIK ZREALIZOWANYCH Z KOLUMNY A
        st.markdown(f'<div class="metric-card"><p>✅ ZREALIZOWANE</p><h3>{liczba_zrealizowanych}</h3></div>', unsafe_allow_html=True)
