...
# ==========================================================
# 1. KONFIGURACJA I PRZYWRÓCONA STYLIZACJA PODSUMOWANIA
# ==========================================================
...
st.markdown("""
    ...
    /* --- PRZYWRÓCONE WYŚRODKOWANE METRYKI --- */
...
if st.session_state.get('show_form', False):
    with st.expander("📝 NOWE ZADANIE", expanded=True):
        ...
        with f_col1:
            n_tresc = st.text_input("Co jest do zrobienia?")
            n_osoba = st.selectbox("Kto odpowiada?", ["Marta", "Sławek", "Agata", "Rafał", "Andrzej", "Pola"])
        with f_col2:
            n_deadline = st.date_input("Na kiedy?")
            n_uwagi = st.text_area("Uwagi dodatkowe:")
        ...
        if c1.button("✅ ZAPISZ", use_container_width=True):
            if n_tresc:
                if dodaj_zadanie_do_arkusza(n_tresc, n_osoba, n_deadline.strftime("%d.%m.%Y"), n_uwagi):
                    st.success("Zadanie zapisane!"); st.session_state.show_form = False; st.cache_data.clear(); st.rerun()
...
with tabs[3]:
    if df_chat.empty or 'ODBIORCA' not in df_chat.columns:
        ...
        if t_input:
            if wyslij_wiadomosc(zalogowany, odbiorca, t_input): st.rerun()
