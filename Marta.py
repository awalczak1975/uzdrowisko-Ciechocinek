...
# ==========================================================
# 1. KONFIGURACJA I STYLIZACJA (NAPRAWA CZATU)
# ==========================================================
...
st.markdown("""
    ...
    .logo-container { text-align: center; margin-top: -70px !important; margin-bottom: 25px !important; }
    .logo-container img { width: 200px; }
    
    /* STYL CZATU - WYMUSZENIE WIDOCZNOŚCI */
    .chat-bubble-container { display: flex; flex-direction: column; gap: 8px; padding: 10px; background: white; border: 2px solid #e2e8f0; border-radius: 12px; margin-bottom: 20px; }
    .bubble { padding: 10px 15px; border-radius: 15px; font-size: 13px; max-width: 85%; color: black !important; line-height: 1.4; }
    .bubble-me { align-self: flex-end; background-color: #eab308; border-bottom-right-radius: 2px; }
    .bubble-other { align-self: flex-start; background-color: #f1f5f9; border-bottom-left-radius: 2px; border: 1px solid #cbd5e1; }
    .bubble-info { font-size: 10px; margin-bottom: 3px; font-weight: bold; color: #64748b; }

    .sticky-user-badge { position: fixed; bottom: 15px; left: 15px; width: 270px; background-color: #eab308; color: #1e293b !important; padding: 5px 10px; border-radius: 6px; font-weight: 800; font-size: 0.75rem; text-align: center; z-index: 9999; border: 1px solid white; }
...
# --- NAPRAWIONA SEKCJA CZATU ---
with tabs[3]:
    st.subheader("💬 Komunikacja Zespołu")
    if not df_chat.empty:
        # Filtrowanie i wyświetlanie chmurek
        ...
        for _, r in historia.iterrows():
            ...
            chat_html += f'<div class="bubble {align}"><div class="bubble-info">{r["NADAWCA"]} • {r["DATA"]}</div>{r["TREŚĆ"]}</div>'
        chat_html += '</div>'
