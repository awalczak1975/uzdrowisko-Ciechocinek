...
st.set_page_config(
    ...
    layout="wide", 
    initial_sidebar_state="expanded" # WYMUSZENIE PANELU BOCZNEGO
)
...
# ==========================================================
# 3. WERYFIKACJA I SIDEBAR (LEWY PANEL)
# ==========================================================
...
# ==========================================================
# 4. WIDOK GŁÓWNY (ZAKŁADKI + PODSUMOWANIE)
# ==========================================================
...
for i, kat in enumerate(kat_list[:-1]):
    with tabs[i]:
        df = pobierz_df(kat)
        if not df.empty:
            # PRZYWRÓCENIE GÓRNEGO PODSUMOWANIA (METRYKI)
...
with tabs[-1]:
    ...
    with c1: msg = st.text_input("Wiadomość...", key="chat_in", label_visibility="collapsed")
    with c2: 
        if st.button("WYŚLIJ 📩", use_container_width=True):
            if msg and wyslij_chat(zalogowany, msg): st.rerun()
    
    st.divider()
    df_c = pobierz_df("Czat")
    if not df_c.empty:
        for _, r in df_c.iloc[::-1].iterrows():
            st.markdown(f'<div class="chat-bubble"><div class="chat-meta">{r["Autor"]} • {r["Data"]}</div>{r["Wiadomość"]}</div>', unsafe_allow_html=True)
