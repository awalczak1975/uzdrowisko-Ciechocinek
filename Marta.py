...
# ==========================================================
# 1. KONFIGURACJA I STYLIZACJA (KOMPAKTOWY PASEK ZALOGOWANIA)
# ==========================================================
...
st.markdown("""
    ...
    /* --- NOWA KOMPAKTOWA ETYKIETA ZALOGOWANIA --- */
    .sticky-user-badge {
        position: fixed;
        bottom: 15px;
        left: 15px;
        width: 270px; /* Szerokość dopasowana do panelu */
        background-color: #eab308;
        color: #1e293b !important;
        padding: 5px 10px; /* Zredukowany padding - pasek jest niższy */
        border-radius: 6px;
        font-weight: 800;
    ...
    .sidebar-divider { border-top: 1px solid #334155; margin: 6px 0; }
    .sidebar-header { color: #eab308; font-size: 0.75rem; font-weight: 800; text-transform: uppercase; margin-bottom: 3px; }
    
    /* ODSTĘP NA DOLE SIDEBARA DLA PAZKA */
    .sidebar-content-spacer { height: 60px; }

    .term-box { background: #334155; padding: 5px 8px; border-radius: 6px; border-left: 4px solid #ef4444; margin-bottom: 4px; color: white; font-size: 0.7rem; }
...
def generuj_kalendarz_html(df_zadania):
    ...
    html = f'<div style="background:white; padding:6px; border-radius:8px; border:2px solid #eab308; font-family:sans-serif;"><table style="width:100%; border-collapse:collapse; line-height:1.2; font-size:11px;"><thead><tr><th colspan="7" style="color:#1e293b; text-align:center; font-weight:800; border-bottom:1px solid #eee; padding-bottom:4px;">{calendar.month_name[now.month].upper()}</th></tr></thead><tbody>'
    for week in cal:
...
with st.sidebar:
    ...
    st.components.v1.html(generuj_kalendarz_html(df_biez), height=170)
