...
def pobierz_polaczenie():
    """Łączy się z Google Sheets, naprawiając błędy formatowania klucza."""
    try:
        if "gcp_service_account" in st.secrets:
            info = dict(st.secrets["gcp_service_account"])
            
            # KLUCZOWA POPRAWKA: Zamieniamy tekstowe \n na prawdziwe znaki nowej linii
            # To eliminuje błąd 'Invalid base64-encoded string'
...
def pobierz_dane_po_indeksie(numer_arkusza):
    """Pobiera dane z arkusza na podstawie jego kolejności."""
...
with tabs[0]: st.dataframe(df_biezace, use_container_width=True, hide_index=True) if not df_biezace.empty else st.info("Brak zadań.")
